...
def calculate_statistics(rec_file, ref_file, options):
    ...
    # output = open('wer_result_wiki_lexicon_%s.txt' %outputpath,'w')
    # substitute_words = open('substitute_words_pair_%s.txt' %outputpath, 'w')
    ...
    while len(i) != 0:
            ...
            for i in changes:
                ...
                if options.v == True:
                    ...
                    if edition == 'S':
                        ...
                        # print '\n%s' %(rec + join_symbol +ref).encode('utf-8')
                        # substitute_words.write((rec + ' ' +ref + '\n').encode('utf-8'))
                    elif edition == 'A':
        ...
        if options.V == 1:
            ...
            # output.write("[II] WER-per-sentence Eds: %d Ref: %d\n" %(v_editions,len(w_j)))
    ...
    if ref_count == 0:
        ...
        if options.cer:
            ...
            tag = "WER"
        statics = float(subs_all + ins_all + dels_all) / ref_count * 100
        stdout.write("%s: %.2f (Ins: %d Dels: %d Subs: %d Ref: %d )" \
                     % (tag, float(subs_all + ins_all + dels_all) / ref_count * 100, ins_all, dels_all, subs_all,
                        ref_count))
        # output.write("%s: %.2f (Ins: %d Dels: %d Subs: %d Ref: %d )" \
    ...
    if options.vocab != None:
        ...
        # output.write(" OOVs: %.2f%%" %(float(oovSubs+oovIns)/ref_count*100))
        stdout.write(" OOVsSubs: %.2f%%" % (float(oovSubs) / subs_all * 100))
        # output.write(" OOVsSubs: %.2f%%" %(float(oovSubs)/subs_all*100))
        stdout.write(" OOVsIns: %.2f%%" % (float(oovIns) / ins_all * 100))
        # output.write(" OOVsIns: %.2f%%" %(float(oovIns)/ins_all*100))
    stdout.write("\n")
    # output.write("\n")

    if options.key_pressed:
        stdout.write("Number of pressed keys: %d\n" % n_pressed_keys)
        # output.write("Number of pressed keys: %d\n" %n_pressed_keys)
    ...
    if options.n > 0:
        ...
        for i in range(len(events) - 1, len(events) - 1 - options.n, -1):
            ...
            # output.write("[Worst-%.2d] %.4f%% %.4f%% - %s\n" %(len(events)-1-i+1, float(n)/ref_count*100,float(acc)/ref_count*100, s.encode("utf-8")))

    # output.close()
    # substitute_words.close()
    return statics
...
def main():
    ...
    cmd_parser.add_option('-e', '--equal-func',
                          action="store", type="string", dest="equal_func", default="standard",
                          help='String compare function=[ '
    ...
    cmd_parser.add_option('-f', '--excp-file',
                          action="store", type="string", dest="excp_file",
                          help='File containing the characters to delete')
    ...
    cmd_parser.add_option('-K', '--number-keys',
                          action="store_true", dest="key_pressed",
                          help='Calcultate the number of keys need to correct erroneous words')
    ...
    rec_file = codecs.open(args[0], "rb", "utf-8", errors='ignore')
    # rec_file = rec_file.replace('\n', ' ')
    ref_file = codecs.open(args[1], "rb", "utf-8", errors='ignore')
    # outputpath = args[0]
